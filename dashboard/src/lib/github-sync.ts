const DEFAULT_REPO = 'Calidad96/calidad-install-eos-scorecard';
const WORKFLOW_FILE = 'sync-install-scorecard.yml';
...
export const GITHUB_ACTIONS_SYNC_URL = `https://github.com/${DEFAULT_REPO}/actions/workflows/${WORKFLOW_FILE}`;

export async function triggerGithubSync(): Promise<{ triggered: boolean; message: string }> {
  const token = process.env.GITHUB_SYNC_TOKEN;
  if (!token) {
    return {
      triggered: false,
      message: `Add GITHUB_SYNC_TOKEN to Vercel, then redeploy. Or run sync manually on GitHub Actions.`,
    };
  }
  ...
  const res = await fetch(
    `https://api.github.com/repos/${owner}/${repo}/actions/workflows/${WORKFLOW_FILE}/dispatches`,
    {
      ...
      body: JSON.stringify({ ref: process.env.GITHUB_SYNC_REF ?? 'main' }),
      cache: 'no-store',
    }
  );
  ...
  if (res.status === 204) {
    return {
      triggered: true,
      message: 'Full Install sync started on GitHub Actions — usually 5–15 minutes.',
    };
  }
