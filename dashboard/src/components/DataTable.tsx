    ...
    >
      <table className="w-full border-collapse text-[13px]">
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr>
              <td colSpan={headers.length} className="px-4 py-12 text-center text-[var(--muted)]">
                No records
              </td>
            </tr>
          ) : (
            rows.map((row, i) => (
              <tr
                key={i}
                className="border-b border-[var(--border)]/35 even:bg-[var(--hover-row)]/40 hover:bg-[var(--hover-row)] last:border-0"
              >
                {row.map((cell, j) => (
                  <td
                    key={j}
                    className={`px-4 py-3 align-middle text-[var(--ink)] ${
                      headers[j]?.align === 'right' ? 'text-right tabular-nums' : 'text-left'
                    }`}
                  >
                    {cell}
                  </td>
